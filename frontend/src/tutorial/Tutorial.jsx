...
export default class Tutorial extends Component {
    ...
    mouseEventInterceptHandler(e) {
        ...
        for (let modal of document.querySelectorAll(".TutorialModal")) {
    ...
    render() {
        ...
        return (
            <div>
                { pageFlagTarget &&
                    <PageFlag ref="pageflag" className="z4" target={pageFlagTarget} bounce={this.state.bouncePageFlag} />
                }
                { portalTarget &&
                    <Portal className="z2" target={portalTarget} />
                }
                { modal && (step.getModalTarget ?
                    <Popover getTriggerTarget={step.getModalTarget} targetOffsetY={25} onClose={onClose}>{modal}</Popover>
                :
                    <Modal style={{ backgroundColor: "transparent" }} onClose={onClose}>{modal}</Modal>
                )}
            </div>
        );
    }
